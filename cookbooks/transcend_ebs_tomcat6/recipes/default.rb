...
remote_file "/root/s3-curl.zip" do
...
bash "unzip s3-curl" do
  user "root"
  cwd "/root"
  code %(unzip s3-curl.zip -d /root)
  not_if { File.exists? "/root/s3-curl" }
end
...
bash "get_web_app" do
  ...
  user "root"
  cwd "/root/s3-curl"
  Chef::Log.info("getting webapp #{s3bucket}/#{s3key}")
...
if configs['PostWaitUrl'].nil?
	Chef::Log.info("No PostWait call required")
else
	http_request "postwait" do
	  action :post
	  url "#{configs['PostWaitUrl']}?Action=PostWait&Hostname=#{node[:fqdn]}&StackId=#{configs['StackId']}&AcId=#{configs['AcId']}&Status=success"
	end
end
